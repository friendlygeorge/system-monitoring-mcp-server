...
export function registerSystemOverviewTools(server: McpServer): void {
  server.tool(
    "system_overview",
    "Get a complete system snapshot: CPU load, memory, disk usage, uptime, and hostname. One call gives you the full picture.",
    {},
    async () => {
      try {
        // Uptime
        ...
        for (const line of meminfo.split("\n")) {
          ...
          if (match) memMap[match[1]] = parseInt(match[2]) * 1024; // kB to bytes
        }
