...
export function registerProcessListTools(server: McpServer): void {
  server.tool(
    "process_list",
    "List top processes by CPU or memory usage. Returns PID, name, CPU%, MEM%, and command. Use for diagnosing what's consuming resources.",
    {
    ...
    async ({ sort_by, limit }) => {
      try {
        ...
        const output = execSync(
          `ps aux ${sortFlag} --no-headers | head -${safeLimit}`,
