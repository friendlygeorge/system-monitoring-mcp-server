...
export function registerCpuInfoTools(server: McpServer): void {
  server.tool(
    "cpu_info",
    "Get detailed CPU information: model, cores, frequency, cache, and current usage per core. Use for capacity planning and performance diagnosis.",
    {},
    async () => {
      try {
        ...
        for (const line of cpuinfo.split("\n")) {
          ...
          if (match) {
            ...
            if (key === "processor") {
              if (Object.keys(current).length > 0) cores.push(current);
