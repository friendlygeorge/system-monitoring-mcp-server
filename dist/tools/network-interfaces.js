...
export function registerNetworkTools(server) {
    server.tool("network_interfaces", "List all network interfaces with IPs, MAC addresses, link status, and traffic stats (bytes in/out). Use for diagnosing connectivity issues.", {
        ...
        try {
            ...
            try {
                ...
                const filtered = interface_name
                    ? ifaces.filter((i) => i.ifname === interface_name)
                    : ifaces.filter((i) => i.ifname !== "lo");
                const result = filtered.map((iface) => ({
                    name: iface.ifname,
                    ...
                    ipv4: (iface.addr_info || [])
                        ...
                        .map((a) => ({ address: a.address, prefix: a.prefixlen })),
    ...
    server.tool("network_connections", "List active network connections (TCP/UDP). Shows local/remote addresses, state, and associated process PID/name.", {
