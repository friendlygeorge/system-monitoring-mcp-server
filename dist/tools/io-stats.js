...
export function registerIoStatsTools(server) {
    server.tool("io_stats", "Get disk I/O statistics per block device: reads, writes, I/O time. Use for diagnosing slow disks or I/O bottlenecks.", {}, async () => {
        try {
            ...
            for (const line of diskstats.split("\n")) {
                ...
                if (name.match(/\d$/) && !name.startsWith("nvme"))
                    continue;
                if (name === "loop" || name === "ram" || name.startsWith("dm-"))
                    continue;
...
//# sourceMappingURL=io-stats.js.map