...
export function registerMemoryDetailTools(server) {
    server.tool("memory_detail", "Get detailed memory breakdown: total, used, free, buffers, cached, swap, available. Shows where memory is actually going.", {}, async () => {
        try {
            ...
            for (const line of meminfo.split("\n")) {
                ...
                if (match)
                    memMap[match[1]] = parseInt(match[2]); // in kB
            }
            const toGB = (kb) => Math.round((kb / 1048576) * 100) / 100;
    ...
    server.tool("top_memory_consumers", "List processes consuming the most memory. Quick way to find what's eating your RAM.", {
        ...
        try {
            ...
            const output = execSync(`ps aux --sort=-%mem --no-headers | head -${safeLimit}`, { encoding: "utf-8", timeout: 5000 });
...
//# sourceMappingURL=memory-detail.js.map