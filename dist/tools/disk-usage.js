...
export function registerDiskUsageTools(server) {
    server.tool("disk_usage", "Get disk usage for all mounted filesystems. Shows total, used, available, mount point, and filesystem type.", {
    ...
    server.tool("inode_usage", "Get inode usage for all filesystems. Inodes track file metadata — if inodes are exhausted, you can't create new files even with free disk space.", {}, async () => {
        try {
            const { execSync } = await import("child_process");
